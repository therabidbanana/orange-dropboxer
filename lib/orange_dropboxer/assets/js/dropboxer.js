$(function(){
	...
	$('.actions .all').click(function(e){
		e.preventDefault();
		$('.doc.file', $(this).closest('.files')).each(function(){
	...
	$('.dropboxer .image.file a').attr('rel', 'fancy').fancybox({
...
function apply_s3_upload(el){
	...
	$(el).each(function(e){
		...
		$(this).s3_multiupload({
			...
			path: '/assets/_dropboxer_/s3_multiupload.swf',
			required: true,
			multi: true,
			element: '<div />',
			...
			onselect: function(array) {
				...
				$.each(array, function(i, info){
					...
					if(parseInt( info.size ) > max_file_size ){
						alert("Too big file - "+info.name+". Must be smaller than " + size_format(max_file_size) + " (was "+size_format(info.size)+")");
						ret = false;
					}
					else{
						
						my_box.append("<div class='file' id='"+info.name.replace(/[\s.]+/g, '-')+"'><span class='progress'></span><span class='file_name'>"+info.name+" ("+size_format(info.size)+")</span></div>");
					}
				});
				return ret;
			},
			onprogress: function(p, key) {
				var my_box = $(this);
				$('#'+key.replace(/[\s.]+/g, '-')+ ' span.progress').css("width", ""+Math.ceil( p * 100 )+"%");
				return true;
			},
